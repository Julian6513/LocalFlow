---
name: privateflow-release
description: Prepare and publish PrivateFlow app releases. Use when the user asks to release PrivateFlow, prepare a new version, bump the PrivateFlow version, write or update PrivateFlow changelog entries, validate a PrivateFlow semver release, create a vX.Y.Z tag, or publish a signed PrivateFlow DMG through the repository GitHub Actions release workflow.
---
...
Use this skill to prepare a PrivateFlow release from the local repository. PrivateFlow releases are semver-tag driven: pushing a tag like `v0.3.1` triggers `.github/workflows/release.yml`, which stamps the app bundle, extracts the matching `CHANGELOG.md` section, builds/signs/notarizes the DMG, and creates the GitHub Release.
...
5. Validate locally before commit/tag:
   ...
   .agents/skills/privateflow-release/scripts/privateflow-release-check.sh <version>
   git diff --check
   make clean
...
8. After GitHub Actions finishes, verify:
   ...
   - `PrivateFlow.dmg` is attached.
   - Release body starts with the matching `CHANGELOG.md` section.
   - A previous app version detects the update and shows What’s New.

## Helper Script

Run `.agents/skills/privateflow-release/scripts/privateflow-release-check.sh <version>` from the PrivateFlow repo root to check release preconditions. It validates semver shape, required files, changelog extraction, workflow trigger basics, and tag availability.
